package com.sixcandoit.firstservice;
...
public class FirstServiceController {
    ...
    /* 설명. @Value와 같이 application.yml 또는 환경설정 값을 불러오기 위해 추가(생성자 주입) */
    ...
    public String healthCheck() {
        ...
        /* 설명. first-service 애플리케이션 여러개 실행 시(스케일 아웃) 실행되는 애플리케이션 확인(포트번호로) */
        /* 설명. Gateway의 lb(로드 밸런싱)를 통해 RoundRobin 방식으로 스위칭 됨을 확인 */
        return "I'm OK port at" + env.getProperty("local.server.port") ;
    }

    /* 설명. Gateway를 거치고 넘어오는 요청 시 RequestHeader에 값이 추가 됨(Gateway)의 필터에 의해 */
    @GetMapping("message")
